    ...
    >
        <v-card >
             <v-toolbar color="#CB3737" dark flat>
                <v-spacer></v-spacer>  
                <v-toolbar-title>
                    {{ `${action === 'updateSchedule' ? 'Schedule' : action === 'addSubjects' ? 'New Subjects' : 'Student\'s List'}` }}
                </v-toolbar-title>
                <v-spacer></v-spacer>
                 <v-tooltip  bottom>
    ...
    watch:{
        ...
        dialog(value){
         ...
         data:{
            ...
            if(this.action === 'updateSchedule' && Object.keys(data).length > 0){
