    ...
    >
        <v-card >
             <v-toolbar color="#CB3737" dark flat>
                <v-spacer></v-spacer>  
                <v-toolbar-title>
                    {{ `${action === 'edit' ? 'Schedule and Teacher' : action === 'addSubjects' ? 'New Subjects' : 'Student\'s List'}` }}
                </v-toolbar-title>
                <v-spacer></v-spacer>
                 <v-tooltip  bottom>
...
export default {
        ...
        dialog(value){
         ...
         data:{
            ...
            if(this.action === 'edit' && Object.keys(data).length > 0){
                let {grade_level, section, days, time, teacher, teacher_id} = data
                this.formData = {
                    grade_level,
                    section,
                    subjects: '',
                    days: days !== "None" ? days.split('/') : '',
                    start_time: time !== "None" ? time.substr(0,5) : null,
                    end_time: time !== "None" ? time.substr(7) : null,
    ...
    methods:{
        ...mapActions("section",["GET_SUBJECTS","GET_TEACHERS","GET_STUDENTS_SELECTION"]),
        setSelections(){
            const payload = {
        ...
        handleSubmit(){
            ...
            if(formStatus){
                ...
                if(this.action == 'edit'){
                    let {days, start_time, end_time, teacher: {id} } = this.formData
                    let data = {
                        days: days.join('/'), 
                        start_time,
                        end_time, 
                    ...
                    this.$store.dispatch("section/EDIT_SCHEDULE", payload)
                }
                 this.closeDialog()
            }
        },
